 ...
 * Icon cache
 * ---------------------------------------------------------------------------
 * Each script's @icon is fetched ONCE, converted to a data: URL, and stored in
 * chrome.storage.local under `ms_icons`. Every later render (refresh, save,
 * toggle, filter…) reads the cached data: URL — zero network. The cache is
 * permanent; a script's icon is re-fetched only when that script is saved.
 * (The options page holds <all_urls>, so cross-origin icon fetches bypass CORS.)
 * ========================================================================= */
